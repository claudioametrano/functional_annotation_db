...
- try use 8 monomeric subunit and see if Alphafold manage to predict something similar to the original pore (the 8 subunits polymerize due to H bonds)
#### TASK2
- Checks dependancies
- tRNA prediction (aragorn)
- rRNA prediction (Barrnap)
- CRISPR repeat arrays (minced)
- Predicting CDS (Prodigal)
- blastp for initial match wit IS (insertion sequence: bacterial mobile element) and AMR (anti microbial resistance) databases
...
Total codon  = 4^3 = 64
Sense codon = 64 -3 stop codons = 61 (3 stop codons)
The **third base of the codon** (often called the **"wobble position"**) can pair with multiple bases in the tRNA anticodon (1st anticodon base), so less that 61 tRNA are often enough. 
The third base for many aa is redundant (different nucleotides get the same aa).
...
Or using the GO term for this Cellular Component
```bash
